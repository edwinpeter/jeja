...
d3.spatialsankey = function() {
  ...
  spatialsankey.ranges = function() {
    ...
    nodes = nodes.map(function(node) {
      ...
      var inflows = links.filter(function(link) { return link.target == node.properties.SUBZONE_C; });
      var outflows = links.filter(function(link) { return link.source == node.properties.SUBZONE_C; });
  ...
  spatialsankey.links = function(_) {
    ...
    links = links.map(function(link){
      ...
      var source_feature = nodes.filter(function(node) { return node.properties.SUBZONE_C == link.source; })[0],
          target_feature = nodes.filter(function(node) { return node.properties.SUBZONE_C == link.target; })[0];
